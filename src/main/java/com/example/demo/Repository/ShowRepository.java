...
@Repository
public interface ShowRepository extends JpaRepository<ShowSetupModel,Integer> {
    Optional<List<ShowSetupModel>> findByShowNumber(Long showNumber);

    @Query(value = "SELECT * FROM SHOW_SETUP WHERE BOOKING_ID = ?1 AND PHONE_NUMBER = ?2",nativeQuery = true)
    Optional<ShowSetupModel> findByIdAndPhoneNumber(Long bookingId,String phoneNumber);
